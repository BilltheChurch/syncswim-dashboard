...
打包日期：20260713  
本批范围：98 张原始帧、222 位运动员。只保留本机姿态模型已有至少一个可靠可见关键点的单人 crop；每张图都可直接看到预标注。 每位运动员是一张独立 crop。
...
5. 等任务准备完成后，在 Task 的 **Actions → Upload annotations**（有的版本写作 **Import annotations**）选择 **CVAT for images 1.1**，上传 `phase_b_remaining_crop_skeleton_seeds.xml`。这一步会预置 **222 个 `swimmer` skeleton**，每张 crop 一个。
6. 打开 Job 后确认共有 **222** 张图片，Objects 列表每张已有一个 `swimmer`。其中 **222** 张已有本机 YOLO pose 模型实际预测出的可见关键点（共 2433 个）；模型未可靠预测的点保持空，不会用猜测坐标填充。双 `foot_index` 也保持空，因为 COCO pose 模型不预测该两点。文件名末尾 `_p0`、`_p1` 等是该原始帧中的第几个已确认 swimmer。
