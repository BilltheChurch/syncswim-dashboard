...
## 问题 #36：把 workstation 搬上线 —— Vercel 托不住"大脑和手"，前端上云、后端留 Mac，以及只有联调才抓得到的那个 bug

**起因**：希望 Emily 去现场只带 M5 + iPad + 手机，连 WiFi 就能用整套 workstation —— 不背开发电脑、不碰命令行。主场景是岸上、训练后赛后复盘。

**第一个要想清楚的现实：Vercel 托不了"大脑和手"。**
直觉是"部署到 Vercel 就上线了"。但 Vercel 是 serverless / 静态平台 —— 没有 GPU、函数有超时、更不能访问本机蓝牙硬件。我们这套系统的三块重活：实时 YOLO 推理（需 GPU、需常驻）、bleak 连 M5（需蓝牙硬件且靠近设备）、摄像头流 —— 没一块能跑在 Vercel 上。所以"整套上 Vercel"物理上不可能。真正能拆的是：**前端（脸）上 Vercel，后端（大脑+手）留在 Emily 的 Mac**，中间用隧道连。

**第二个死结：iPad 连不了蓝牙。**
本想让 iPad 浏览器直接用 Web Bluetooth 连 M5。查证后发现 iOS 有个特殊规矩 —— 所有浏览器（包括 App Store 里的 Chrome）都被强制套用 Safari 的 WebKit 内核，而 WebKit 至今不支持 Web Bluetooth。也就是 iPad 上没有任何常规浏览器能连蓝牙。解法反而更简单：**蓝牙留在 Mac 上用 bleak（现有代码不动），iPad 只当显示+控制的瘦客户端** —— 一举绕开死结。

**同源假设的改造。**
旧前端是"前后端同进程"，所有请求走相对路径 `/api/...` + `location.host` 派生 WebSocket。前端上 Vercel 后这些会打到 Vercel 域名而非 Mac 后端，全断。本来要改 34 处 fetch + 2 处 WS + video/pdf/keyframes。与其逐处改，用了更聪明的办法：**在 app.js 顶部拦截 `window.fetch`，凡 `/api/` 开头的请求自动加后端域名前缀** —— 34 处 fetch 一行没动；只有非 fetch 的 `<video src>`/`<img src>`/WS 手动包了 helper。本地运行时后端地址为空=同源，行为和改造前完全一致；上 Vercel 时由一个"配置链接"把隧道域名 + token 存进 localStorage。一份代码，两种部署都跑。

**只有联调才抓得到的那个 bug。**
本地用 FastAPI TestClient 测，token 鉴权和 CORS 都"通过"。但真正用隧道 + 浏览器跨域联调时，前端所有请求挂了。根因：浏览器跨域带自定义请求头（我们的 `X-SyncSwim-Token`）时，会**先发一个不带 token 的 CORS 预检 `OPTIONS` 请求**；而我写的 token 中间件把它当成"无 token"拦成 401，浏览器据此认定跨域被拒，真正的请求根本不发。**这个 bug 本地同源测试永远测不出来** —— 因为同源没有预检。修复是让中间件放行 OPTIONS。教训：单元测试的"绿"不等于真实环境的"通"，跨域、隧道、真实浏览器这些只有端到端联调才暴露。

**一路上的其他坑（每个都教了点东西）：**
- **后台进程 SIGABRT（exit 134）**：把后端做成后台服务跑，一启动就崩。根因是 macOS 的隐私权限（TCC）—— 后台进程没有蓝牙授权，CoreBluetooth 直接 abort。加了个 `SYNCSWIM_NO_HARDWARE` 开关跳过蓝牙/摄像头。意外收获：这个开关顺便实现了"纯赛后复盘模式"——任何一台 Mac 不接 M5 也能打开看历史报告。
- **cloudflared 隧道连不上**：默认走 QUIC（UDP），在受限网络里反复超时。换成 `--protocol http2`（TCP）立刻通。
- **"站点挂了"其实是本机网络在骗我**：换肤版部署后，从开发机 curl 和无头浏览器都连不上 syncswim.vercel.app，TLS 直接报错，差点以为部署坏了。但 `vercel ls` 明明显示部署 Ready。一查 DNS —— 域名被解析到 `198.18.3.97` 这个基准测试保留段的伪 IP，根因是这台 Mac 上 Tailscale 的系统网络扩展（即使应用层已停）仍在劫持 `*.vercel.app`。用 `curl --resolve` 强制走真实 Vercel IP 一试就通，确认站点完全正常。教训：验证失败时先分清是"被测对象坏了"还是"测量环境坏了" —— 这和做生物力学测量时"先校准仪器再信读数"是同一个道理。

**UI 重做（深色 Frontier 品牌）。**
赛后复盘是这台机器的核心用途，UI 整套重做成深海蓝 + 品牌金的深色风格。发现旧样式表本就是深色+蓝+金体系，80% 品牌化只需改 `:root` 里 31 个 CSS 变量。但有两件"换肤改不到"的事：① **canvas 画的图表**（雷达、时序、骨架）颜色写死在 JS 里、不读 CSS 变量，得单独改；② **choreography 卡**用了一个根本没定义的变量 `var(--surface)`，一直靠 fallback 渲染成白底 —— 深色页面上格外刺眼，重做时才发现这个潜伏 bug。顺手把它做成带状态徽章（已验证/有保留/探索性/已排除）的金边卡片，把"诚实标注每个变量的可信度"这个项目最有价值的科学态度，做成了视觉上最突出的一块。

**结果**：前端上线 `https://syncswim.vercel.app`；后端黑盒自启 + 隧道脚本就绪（待 Emily 现场 Mac 实跑）；全链路端到端联调通过（含浏览器实测）。Emily 现在去现场，理论上只需要带 M5、iPad、手机。

### 一行总结
**"整套上 Vercel"是不可能的 —— serverless 托不了 GPU 推理和蓝牙；想清楚"前端上云、后端留 Mac"才是对的拆法。而真正能让前端跨域连上后端的那个 bug（CORS 预检被鉴权拦 401），本地同源测试根本测不出，只有真实隧道联调才暴露。**

---

> 本文档随项目进展持续更新。每次遇到有价值的技术问题都会追加记录。

## 问题 #37：自训 detector 的数据飞轮 —— 从"通用模型 1% 召回"到"v2 98%"，以及一路上的工程坑

### 背景
通用 YOLO 在水域花游只有 ~7% 召回（#33 量化）。Emily 现场用的 yolov8n-pose 实测在 set_008 单人视频只有 **1% 检测率** —— 基本全瞎。必须自训 detector。

### 数据飞轮闭环
1. **v1 标注的"丢失"**：发现"训过却找不到"的根因不是忘了 commit，而是 `.gitignore` 故意排除 `phase_*/labels/`（训练数据不进 repo 的设计）。`git add -f` 强制入库 Emily 标注的 150 帧。
2. **v1**：150 帧 raw clips → mAP50 0.81，set_008 检测率 70%、ID 通胀 11×。
3. **v2 数据扩充**：用 v1 detector 预标注 set_004~008（飞轮：v1 帮标 v2），CVAT 修正 200 帧。合并 350 帧 → mAP50 **0.915**，检测率 **98%**、ID 通胀 3×。

### 三个把我卡住的工程坑
- **MPS 训练崩溃**：torch 2.11 的 Apple MPS 后端训练中 `EmbeddingBag kernel index out of bounds`，跑几 epoch 必 abort。换 `--device cpu` 稳定跑完。教训：**MPS 训练在当前 torch 版本不可靠**。
- **会话 teardown 杀训练**：AI 后台启动的训练进程在会话 idle 时被系统回收（卡在 1/17/32 epoch）。解法：**让训练在用户自己终端 nohup 跑** —— 用户终端的进程不受 AI 会话生命周期影响。
- **多行命令粘贴卡住**：`\` 续行的命令粘进终端最后一行不执行 → 改单行 `;` 连接。

### 标注质量的反复
v2 第一版有 59 帧漏标（运动员在游动但没框）+ 10 帧误标看台。**关键认知**：detector 要"见人就框、不管动作" —— 把有人的帧标空等于教模型"这种画面没人"，比不标更糟。重标后 200 帧全单人干净。

### 系统优化（接在 detector 之上）
- **单人锁定**：复盘锁最高分单框 + 固定 ID=1 → ID 通胀 3×→**1×**。
- **IMU 融合**：脚踝 IMU 校准 + 腿部动力学。**视觉丢腿 84% 帧时 IMU 全程连续** —— 双模态相对纯视觉的根本差异化，且不受水下折射影响。
- **脚尖 hybrid**：YOLO COCO-17 架构无脚尖，绷脚无法评。v2 框 + MediaPipe 33 点 → 脚尖 0%→12%。

### 一行总结
**通用模型在目标域烂到一定程度时，唯一根治是自己标数据训模型；而工程上真正耗时间的不是算法，是 MPS 崩溃、会话 teardown、标注质量这些"周边坑"。**

---

## 问题 #38：从"一堆数字"到"该练什么" —— AI 训练报告，以及为什么故意不用 LLM

### 触发问题
系统已经能算 16 项指标 + 综合评分 + 4 维度分组，但真人试用后暴露一个断层：教练/运动员/家长看到 `姿势稳定性 30.9 deg/s [需改进]` 这行，**不知道下一步该干什么**。指标是"诊断"，缺的是"处方"。产品价值不在于再多显示一个数字，而在于把数字**翻译成动作**。

### 设计决策：规则引擎，而不是 LLM
最直觉的做法是把指标喂给一个大模型让它生成建议。我们**故意没有**，选了纯规则引擎（`dashboard/core/advice.py`：16 指标 × `(问题描述, 训练方法)`，按扣分严重度排序，前 2 条标 ★ 优先，达标项列为亮点）。理由：
- **可解释 / 可追溯**：每条建议都能指回"哪个指标、超了哪个 FINA 阈值"，教练能验证，不是黑箱。
- **不会幻觉**：给运动员的训练建议是**安全问题** —— LLM 偶发编造一个错误动作，代价远高于"少给一条建议"。
- **离线 / 零成本 / 零延迟 / 可复现**：泳池现场常没有好网络；同样输入永远同样输出，方便回归测试。
- 依据是 Edriss 2024 / Yue 2023 的相关系数方向 + 花游训练常识，写死在字典里，可审查。

### 单一事实源
同一个 `build_advice()` 被**前端复盘卡片**和 **PDF 报告第 3 页**共同调用 —— 网页看到的和导出的完全一致，改一处两处都变。前端保留 emoji（✅📈🎯），PDF 因 matplotlib 字体缺 emoji 字形而去掉（踩了 `Glyph 9989 missing` 的坑），文案内容不变。

### 实测（三种数据形态都成立）
- `set_006`（横屏、纯视觉、无 IMU）：3 条建议，全是视觉几何项（举腿高度/肩膝对齐/膝伸展）。
- `set_004`（竖屏、视觉 + 双 IMU）：5 条建议 + 双模态，最完整。
- `set_001`（纯 IMU、无视觉）：6 条建议，全是时序/动力学项 —— 视觉指标诚实显示"无数据"而非瞎编。

### 一行总结
**能测量 ≠ 有用；把 30.9 deg/s 翻译成"练靠墙倒立静态保持"才是产品。而在"给人的训练建议"这种安全敏感场景，一个可解释、不会幻觉的规则引擎，胜过一个更聪明但偶尔胡说的大模型。**

---

## 问题 #39：给 Emily 打更新包 —— 部署的魔鬼在"接缝"，不在算法

要把新功能（AI 报告）+ 新检测器（v2）打成一个包发给 Emily 双击更新，看似 `bash build-kit.sh` 一行完事，实际踩了两个典型的"部署接缝"问题。

### 坑一：打包脚本静默漏了训练好的权重
`build-emily-kit.sh` 用 `git archive HEAD` 打代码包，而训练产物 `best.pt` 被 `.gitignore`（`runs/` + `*.pt`）排除——所以 git archive 打不进，脚本本身又没有单独 `cp` 它。结果：`config.toml` 指向 `runs/.../best.pt`，但包里根本没有这个文件。更阴险的是**不报错**：检测器加载处 `if path and os.path.exists(path)` 找不到就**静默退回通用模型**，而通用 nano 在水下召回仅 ~1%。Emily 装完"能跑但检不到人"，却没有任何报错指向真因。
**教训**：gitignored 的产物必须显式打包，且落位链路（打进包 → 传到机器 → 落到 config 期望的路径）要**端到端**验证，不能只验证"包生成成功"。

### 坑二：用户在网页改的设置，被更新覆盖
摄像头 URL 每次现场都可能变（DroidCam 的 IP 随网络变）。系统本来就支持在网页设置页改，改完 `save_config()` 写回 `config.toml`。但 kit 更新用 `code.zip` **整体覆盖** `config.toml` → 用户改的 URL 被冲回模板值，每次更新后都要重填。
根因是**用户运行时设置和代码默认配置混在同一个文件里**，而更新机制是"整体覆盖代码"。解法是经典的**关注点分离**：代码默认留 `config.toml`（随更新走），用户设置分到 `data/camera_override.json`（更新从不碰 `data/`），`load_config()` 加载时合并、以 override 优先。改一次，永久生效。

### 一行总结
**部署的魔鬼从来不在算法，而在接缝：权重到底进没进包、用户改的设置会不会被下次更新冲掉、失败时是报错还是"静默退化"。把这些接缝一个个焊死，才谈得上"能交给别人用"。**

---

## 问题 #40：远程机器"变卡 + 检测不到人" —— 不猜，用基准测试把"感觉慢"变成数字

Emily 反馈她电脑上 FPS 掉到 13、非常卡、还识别不到人。我不在她机器上，这类"远程性能问题"最容易陷入瞎猜。用了三步把它钉死。

### 第一步：先排除自己刚改的东西
我上一版给 `load_config()` 加了读文件的逻辑。第一反应是怀疑它在每帧热路径造成 IO 拖慢。于是去读调用链：`load_config()` 在 `camera_manager.py:390` 只在检测器**初始化**时读一次（进入帧循环之前），帧循环体（`while` 里）只调 `detect()`，没有我的 IO。**排除自己的改动 —— 靠读代码，不靠感觉。**

### 第二步：基准测试定量定位
在本机（比 Emily 电脑快）对检测器单帧推理计时，跑三种配置对比：

| 配置 | ms/帧 | FPS |
|---|---|---|
| v2 hybrid + num_poses=8（当时的 config） | 110 | **9.1** |
| 单模型（无 v2 检测器） | 31 | 31.9 |
| v2 hybrid + num_poses=1 | 40 | 24.7 |

根因一目了然：**hybrid 是两阶段（检测器出框 + 对每个框各跑一次 pose），config 里 `num_poses=8` 让它每帧跑 8 次 pose**。连我的快机器都只有 9 FPS，Emily 电脑掉到 13（她可能更少人）完全吻合。降到 `num_poses=1` → 快 2.7 倍。"感觉慢"变成了"9 vs 25 FPS"。

### 第三步：区分"模型坏了"和"用错场景"
注意基准里 v2 在泳池帧**检测到 5 人** —— 模型是好的。Emily"识别不到人"是因为她在**岸上/室内**对着摄像头测试，而 v2 是专门为"水下游泳者"训练的，认不出岸上的普通人是**预期行为**。这不该靠调参解决，而该给用户一个**开关**：泳池现场用 v2，普通场景切通用模型（还更快）。于是在设置页加了检测模型切换，复用"设置存 data/、不被更新覆盖"那套机制。

（顺带还揪出一个隐患：config 指向 `yolov8n-pose.pt`，但 kit 只打包了 `yolov8s-pose.pt` —— 缺文件会让检测器加载失败。一并修正为一致。）

### 一行总结
**性能问题不要猜：先用读代码排除自己，再用基准测试把"卡"变成可对比的数字；而"检测不到"常常不是模型坏了，是用错了场景 —— 给用户一个切换开关，比闷头调参实在得多。**

---

## 问题 #41：一个 NaN 干掉整份报告 —— "加载失败"背后的 allow_nan

Emily 报两件事：新录的 set 一直"加载失败"（旧版录的能打开），以及 FPS 卡在 13 换模型也没用。

### 症状不对称是最强线索
"新录的失败、旧的正常" 说明不是代码路径坏了（否则全都打不开），而是**新数据里有某种旧数据没有的东西**。我先用 `import_video` 在本机以当前代码生成新 set → 加载正常（HTTP 200）；又手工构造"空 IMU + 5 帧"的极短录制 → 仍正常。本机复现不出来。

### 从"前端为什么显示这句话"倒推
前端那句"加载失败"只出现在 `fetch(report)` 的 `catch` 里 —— 也就是**请求抛异常或响应不是合法 JSON**。顺着这条线查序列化，找到真因：

```python
JSONResponse({"score": float("nan")})
# ValueError: Out of range float values are not JSON compliant: nan
```

Starlette 的 `JSONResponse` 用 `allow_nan=False`。**报告 dict 里只要有一个 NaN，整个响应就在序列化时抛异常 → 500 → 前端只看到"加载失败"。** 而 NaN 在稀疏数据里太容易出现了：帧数太少、单样本求方差、除零。旧的完整录制算不出 NaN，Emily 低帧率的短录制算得出 —— 症状的不对称完全解释通了。

同一类隐患还有一处：损坏/未转码完的 `video.mp4` 会让 `cap.get(CAP_PROP_FRAME_COUNT)` 返回 nan，`int(nan)` 直接 ValueError，同样 500。**视频元数据不该有权阻断整份报告。**

修法是"局部失败不升级为整体失败"：`_json_safe()` 递归把 NaN/±Inf 换成 null，视频探测加 isfinite + try/except。一个指标算不出来就显示"无数据"，评分、其余指标、AI 建议照常出。

### 顺带：可诊断性本身就是功能
这次卡了很久，根本原因是前端把真实错误**吞掉了**（`catch {}` 连 error 都没接）。改成显示 HTTP 状态和解析错误后，同类问题以后 Emily 念一句话就能定位，不用远程翻 `/tmp/syncswim.log`。

### 顺带二：读流器的 O(n²)
查 FPS 时读到 MJPEG 解析：每次 `stream.read(4096)`，然后 `buf += chunk`（整块重拷）+ 对**整个 buffer** 全量 `find`。一帧 500KB 就是 125 次拼接和 125 次全量扫描 —— 典型 O(n²)，**帧越大越慢**（实测分辨率翻倍，耗时 52→255ms）。改成 64KB 分块 + `bytearray` + 记住扫描位置后快 35–60×；并且一轮只解码**最新**一帧（主循环只用最新帧，解码中间帧纯属浪费，还和推理线程抢 GIL）。

而 FPS 的真相是：实测 hybrid 42ms(23FPS)、通用单模型 15ms(65FPS)、高清图像搬运仅 5.5ms —— **本机没有任何环节能解释 13 FPS**。两种模型都是 13，就说明瓶颈不在模型，而在那台机器的算力或摄像头供帧率。所以我加了 `skipped_frames` 计数：丢弃的过期帧 >0 说明摄像头比我们快（瓶颈在本机），≈0 说明摄像头本身只给这么多帧。**与其继续猜，不如让系统自己报告瓶颈在哪。**

### 一行总结
**局部失败不该升级成整体失败：一个算不出的指标（NaN）不该让整份报告 500。而当远程问题复现不出来时，正确的投资是"让系统自己说出真相" —— 暴露真实错误、埋下能区分瓶颈的计数器，比反复猜测高效得多。**
