...
7.4 的目标是"用真实游泳素材微调 YOLOv8-pose"。但我们目前**还没有**真实素材 — Tim 老师决定先把整个阶段七的代码框架打完再上传素材。
...
所以 7.4 这一阶段不动模型，只准备**完整可跑的基础设施**：脚本、配置、文档。等 Tim 老师一上传素材，能 1 行命令跑完整流程。
...
Tim 老师决定先把阶段 7 全部完成 + 后续若干小特性，再上传真实素材做 dogfood。但**当下还有一摞历史训练录像可以先用**。问题是：现有 pipeline 的入口只有"实时录制"——MJPEG 摄像头流 + BLE IMU + Button A 触发 → 落盘 set。视频文件没有任何入口能进系统。
...
第一版**故意 hardcode** 而不是塞进 `config.toml`：dogfood 之前我们不知道哪条规则真的有用、阈值合理与否。**让真实使用淘汰假设**，等 Tim 老师用过几周再决定哪些规则进生产、哪些阈值要调。
...
阶段 8 全部完成后，Tim 老师提供了**三段真实花泳训练视频**做 dogfood：32s 横屏 5 人同框 + 21s/22s 两段竖屏 2-3 人。这是项目第一次接触真实场景数据。直觉是"应该没大问题，可能要微调"，**结果数字让 fine-tune 从 nice-to-have 变成必须**。
...
2. **数字比体感更服人，但要先有体感才知道找哪个数字**：Tim 老师的"还是闪烁"是触发器，我去查 99 个 ID / 5 个运动员才把"闪烁"翻译成 "19.8× ID inflation"。**体感 → 数字 → 决策**，缺一环都不准。
...
### 背景
DEVLOG #33 把"必须 fine-tune"从直觉变成数字（19.8× ID 通胀）。决策做完，下一个问题是：**Tim 老师坐下来标注那一刻，他能不能立刻开干？**

如果工具链没准备好：

1. 没有抽帧脚本 → Tim 老师得手写一个
2. 没有 train/val 拆分约定 → 同视频相邻帧分两边，val mAP 虚高 30%（DEVLOG #28 的旧账，每个新人都会踩）
3. 没有 detector 训练脚本 → Tim 老师得现学 ultralytics CLI 参数，调出 1280 imgsz / batch 8 这些 dogfood 验证过的甜蜜点
4. 没有上线 hook → 训出 best.pt 后还得自己改 yolo_pose.py 让它生效

任何一步缺失，**1-2 小时的标注活会膨胀成 3-4 小时的"标注 + 摸索"**。本 PR (#8) 把所有"非标注"步骤都做成 `python tools/xxx.py` 一行命令，让 Tim 老师的时间 100% 花在唯一不能自动的环节：**画框**。

### 关键设计决策

#### 1. 抽帧脚本不做预标注
现成的 `tools/preannotate.py`（Phase 7.4 产物）会用 COCO YOLO 跑预标 → 人工修正。看似省事，但 #33 已经量化：COCO 在水中花泳召回 ~7%。预标 = 漏掉 93% 的运动员。**Tim 老师看着空白预标的图反而要花更多时间确认"是真没人还是模型瞎"**。
...
改稿原因：**Tim 老师标到第 30 帧时一定会停下来问"这种半身入水的我标不标？"** 如果答案藏在文档第 7 步里，他要么不标（数据漏），要么瞎标（数据脏）。
...
### 一行总结
**让人工活的入口尽量窄、尽量明确**。如果 Tim 老师 1-2 小时能搞定 150 帧标注 → 训练 → 看到 mAP ≥ 0.70 的绿色 ✅，整个 Phase A 闭环走完，那本 PR 就成功了。

### 路线图
- [x] 9.1.0 工具链 + 文档（本 PR #8）
- [ ] 9.1.1 Tim 老师完成 ~150 帧标注
...
## 问题 #36：Emily v2 实测 — "满屏错骨架 + 2fps" 的三层根因，与录制/推理解耦

### 背景
Emily 拿 v2 detector 部署包做了第一轮实测（6 个 Set：set_011 办公室冒烟 + set_012~016 泳池实拍，2026-07-27/28），微信发回归档。反馈两句话：**"大部分骨架都是错的"** + **"预览和录制帧数都非常低，非常卡"**。

逐 Set 量化分析（解码全部视频帧 + 解析 landmarks_multi.jsonl）：

| Set | 场景 | 名义fps | 有效fps | 重复帧中位 | 核心12关节 conf 均值 | conf 中位 |
|-----|------|--------|--------|-----------|---------------------|----------|
| 011 | 办公室 | 25 | 4.1 | 6× | 0.52 | 0.66 |
| 012 | 泳池 | 25 | 2.7 | 9× | 0.46 | 0.45 |
| 013 | 泳池 | 25 | 3.3 | 7× | 0.44 | 0.46 |
| 014 | 泳池 | 25 | 4.0 | 6× | 0.37 | 0.38 |
| 015 | 泳池 | 25 | 1.9 | 14× | 0.17 | **0.012** |
| 016 | 泳池 | 25 | 2.1 | 13× | 0.14 | **0.003** |

两个症状各自有三层根因，全部确认：

### 症状一：骨架错 — COCO keypoint 的"水下幻象"到达了用户眼前

1. **主因（已知天花板）**：v2 只微调了 **detector（bbox）**，keypoint 仍来自 COCO 预训练 `yolov8s-pose`。花泳的倒立/头入水/半身水下姿态在 COCO 里不存在 —— 模型永远输出 17 个坐标，对看不见的关节**发明**一个"合理"位置。set_015/016 的 conf 中位 ≈0 就是 hybrid 模式的铁证：v2 detector 框得很稳（track ID 全程只有 1 个，ID 通胀彻底没了——**v2 的既定目标其实达成了**），但框内 COCO pose 输出全零/幻觉。
2. **放大器一（crop 强出骨架）**：HybridSwimmerDetector 对 crop 跑 pose 用 `conf=0.1, max_det=1` —— 故意压低是为了水中召回，但没有任何下游过滤，幻觉直接进入 JSONL/CSV/前端。办公室场景（set_011）骨架被压扁成 bbox 内一团乱线，就是 crop-pose 幻觉的典型形态。
3. **放大器二（前端阈值过松）**：`app.js` 画骨线的阈值是 `vis > 0.3`、关节点 `vis ≥ 0.2`，而幻觉点 conf 常落在 0.3~0.5 —— 大量幻觉过线被画出来。

**修复**（本次）：
- `yolo_pose.py` 新增 `_apply_kpt_gate()`：conf < `kpt_min_conf`（默认 0.35，config 可调）的关键点在**源头置零** —— 不进 recorder、不进前端、不污染 Phase B 预标注。两个 detector 类都套用。
- `app.js` 新增 `BONE_MIN_VIS=0.45` / `DOT_MIN_VIS=0.35` 常量，替换 3 处骨线 + 3 处关节点的散落阈值。
- **诚实边界**：门控只能"少画错的"，不能"画对的"。办公室那种高置信度幻觉（conf 0.7+ 但位置错）过滤不掉 —— 骨架真正落到运动员身上，只有 **Phase B keypoint 微调**（Emily 标注中）能解决。

### 症状二：卡 — 同步架构让推理速度拖垮整条链路

1. **架构根因**：`CameraManager._run` 是同步循环：读帧 → **推理（阻塞）** → JPEG → `_latest`。预览帧率 = 录制帧率 = 推理帧率。
2. **硬件放大器**：Emily 是 Intel Mac —— config 里 `yolo_device="mps"` 在 Intel 上不可用，warm-up 异常后静默回落 CPU；hybrid 模式每帧 = 1 次 detector pass + N 次 crop pose pass，CPU 上合计 250~500ms → 2~4fps。
3. **录制放大器**：`_vision_writer_loop` 以 25Hz 轮询 `get_latest()`，无"新帧"判断 —— 同一帧被重复写 6~14 次进 video.mp4，同时 landmarks 行同样重复。视频名义 25fps、有效 2~4fps。

**修复**（本次）：`camera_manager.py` 解耦为双线程 ——
- **帧通路**（`_run`）：MJPEG（新增 seq 计数防重复消费）→ 旋转 → JPEG → `_latest`，跟满摄像头帧率；
- **推理通路**（新 `_infer_loop`）：永远只消费最新帧，跑多慢都只影响骨架刷新率；后端 init 失败（模型缺失）只损失骨架，**预览/录制照常** —— 这在旧架构下会直接黑屏。
- `_latest` 新增 `frame_seq/frame_ts/pose_seq/pose_ts`，pose 结果整体替换（不 in-place mutate），帧循环合并进每帧 dict，无撕裂读。
- **generation 令牌**（多智能体评审揪出的隐患）：`set_url` 触发 stop→start 重启时，若旧推理线程正卡在长推理里（CPU 一帧可达数秒 > 3s join 超时），`_running` 翻回 True 会让旧线程"复活"，与新线程竞争同一模型对象。修复：`start()` 递增 `_gen`，两个循环都携带出生代数，代数不符立即退出、且绝不发布旧代 pose。
- **pose_rot 朝向戳**：旋转 90/270° 切换会交换归一化坐标的长宽基——旧朝向的骨架画到新朝向帧上不只是"旧"，是几何错误。修复：旋转 setter 立即清 `_pose_cache`；pose 发布时带 `pose_rot`，帧循环拒绝合并基不符的 pose（含在途推理清缓存后才发布的窗口）。
- **数据溯源**：解耦后骨架天然滞后于视频帧（慢机器上滞后 5~10 帧），`landmarks_multi.jsonl` 每行新增 `frame_seq`/`pose_seq`（向前兼容，旧文件无此字段），离线分析可量化并重对齐滞后，不再把旧骨架无声地归给新帧。
- **评审阶段的其余收尾**：hybrid 的 `swimmer_botsort.yaml` 缺失时回退 ultralytics 内置 botsort.yaml 并打警告（原来会静默全帧零输出；注意该文件目前 untracked，**打 kit 前必须 git add data/tracker_configs/**）；`tools/import_video.py` 新增 `--kpt-min-conf`（0 = 关门控回旧行为）；姿态后端 init 失败通过 `pose_error` 字段透传，实时页 badge 显示"姿态后端故障"而不是无辜的"无人"；前端标签锚点阈值统一到 `DOT_MIN_VIS`（不再出现"悬空 #ID 牌但无点无骨线"）。
- **刻意不做的**：writer 的 25Hz 节拍**保留** —— 固定节拍复写"最新帧"才能保证视频 wall-clock 时长与 IMU 时间轴对齐（阶段三 8.8ms 同步精度的前提）。解耦后最新帧以摄像头帧率更新，重复帧自然消失；相机断流时的重复帧反而是正确行为（保时长）。若改成"来一帧写一帧"，WiFi 抖动 2 秒会让视频凭空快 2 秒，IMU 对齐全毁。

### 教训
1. **"模型不好"的用户反馈里往往埋着一半架构问题**。Emily 说"视觉模型不理想"，实际上骨架错是模型（Phase B 未训），卡是架构（同步推理）—— 若只回一句"等 Phase B"，就把 2fps 的录制体验也归罪给了模型。
2. **有效帧率 ≠ 名义帧率**。ffprobe 看 25fps 一切正常，逐帧 diff 才暴露 6~14× 重复。质检脚本已沉淀（`analyze_emily_sets.py` 思路：unique-frame ratio + conf 分布 + track ID 数三件套）。
3. **低置信度输出不过滤 = 把模型的不自信转嫁给用户的眼睛**。conf=0.1 的召回优化必须配对下游门控，否则"宁可错画不可漏画"。

### 验证
- 新增 `tests/test_camera_decouple.py` 6 项：关键点门控 3 项（低 conf 置零 / 阈值边界 / 门控可关）+ 解耦 3 项（**后端 init 失败帧照常流动** / 慢推理下帧数超推理次数且 pose 滞后不超前 / stop 干净收尾）。
- 全套件 3 连跑稳定：94 passed；16 个失败均为本次改动前已存在（test_scaffold 的 requirements 断言、test_vision_angles 缺列回退等，与本次无关）。

### 路线图影响
- [x] 录制/推理解耦 + 关键点门控 + 前端阈值（本次）
- [ ] Emily 侧配置：`yolo_device = "cpu"`（Intel Mac 显式声明，跳过 mps 失败重试）；打包 kit v3 zip 推送
- [ ] **骨架真正变对：等 Phase B**（Emily 53-crop 单人包标注中 → freeze backbone 只训 pose head → 水下点 visibility=0）

---

## 问题 #37：架构转向 — 砍掉实时推理，视觉全部离线后处理

### 背景
Tim 老师看完 #36 的诊断后拍板了一个重大角色改变：**抛弃所有实时段推理和分析**。前端实时页只展示摄像头画面 + IMU 数据；视觉方案留给录制结束后的分析阶段，跑重权重、跑更久，确保准确性。

两个数据支撑这个决定：
1. 实时推理在 Emily 的 Intel Mac 上只有 2-4fps（#36），即便解耦后也只是"低刷新率的 COCO 幻觉骨架"——对教练没有实际价值，还占满 CPU。
2. 同一段视频（set_013，294 帧），录制时实时路径只有 **29.6%** 的帧检出人；离线重配置（hybrid v2 + imgsz 1280）**98%**。不是模型不行，是实时预算喂不饱它。

顺带纠错：#36 里"办公室场景幻觉重"是误判——总统大人指出 set_011 骨架基本正确。复核数据：上半身 conf 0.9+ 且位置对，髋部 0.3 是被桌子遮挡后的合理估计。幻觉问题只针对泳池姿态（倒立/入水）。**教训：给"错误"下结论前先分关节看 conf 和位置，别把"遮挡下的合理估计"和"幻觉"混为一谈。**

### 关键设计决策

#### 1. 时间戳是对齐锚点，重分析必须继承而不是重造
录制期间 writer 照常写 vision.csv / landmarks.csv（空姿态 + **真实 wall-clock 时间戳**）。离线分析重写这些文件时**逐帧复用原时间戳**（vision.csv 与 video.mp4 1:1，#13 的不变量在这里第三次发挥作用）。这样 IMU↔视觉对齐在重分析后原样保留——如果离线分析自造 0 基时间轴，阶段三的 8.8ms 同步精度就全毁了。无原时间戳的（旧 set / 导入视频）回退 `first_ts + i/fps`。

#### 2. 原子重写：分析崩在第 200 帧不能毁掉一个好端端的 set
三个文件全部写 `.tmp` 再 `os.replace`。任何异常 → 删 tmp、原文件纹丝不动。分析页读到的永远是"完整旧数据"或"完整新数据"。

#### 3. 单进程同时只跑一个分析
重模型吃满 CPU。注册表拒绝并发分析（409 + 提示哪个 set 在跑），保护可能同时进行的录制和面板响应。

#### 4. conf 智能默认 — 同一个参数在两种模式下含义相反
实测（set_013）：conf 0.15 在 hybrid 模式下检出 98% 但 27 个 track ID（浮标/反光进来当人）；0.35 检出 95.6%、21 个 ID。而单模型 COCO 模式下 0.35 会扔掉大部分真游泳员（#33）。所以 `resolve_analysis_config()` 按模式选默认：有 swimmer_detector → 0.35，无 → 0.15，config 显式值可覆盖。**教训：被"低 conf 保召回"救过一次，不代表它在新检测器下还是对的——阈值的语义跟着模型走。**

#### 5. 前端做减法而不是重构
`pose_backend = "none"`（新默认）时 `preloadCameraState()` 隐藏 5 个视觉组件；ws 推送里 landmarks 恒为空数组，骨架绘制路径自然不触发。legacy 模式（yolo/mediapipe）完整保留——一行 config 可回退。分析页新增「视觉分析」按钮 + 进度轮询，完成自动刷新报告。

### 验证
- 单测 17 项全绿（offline_vision 10 + camera_decouple 7，含 backend=none 用例）
- 真实端到端：Emily set_013 离线分析 56s/294 帧（MPS），时间戳保留验证通过
- ID 通胀在这段新泳池素材上仍有 21×（v2 训练集没见过这个池子的机位）——留给 9.3 数据扩充
- 15-agent 多视角评审确认并修复 8 项，最重的一个是 **critical**：`pose_backend=none` 的
  新录制 vision.csv 角度恒 0 → `np.corrcoef` 出 NaN → Starlette JSON 编码（allow_nan=False）
  直接 500 → 分析页整页挂掉、「视觉分析」按钮都看不到——新架构的主流程会被自己录的数据堵死。
  修复：correlation 只取 visible==1 行 + 零方差/非有限守卫。其余修复：行数漂移时真正全合成时间轴
  （原实现"口头合成实际沿用"，会产生时间戳倒退）；解码不足 2% 容差即拒绝提交（防止截断视频
  静默覆盖唯一一份旧 landmarks）；sessions 缓存改为任务完成时失效；录制中启动分析返回 409；
  前端轮询器跨 set 导航不再劫持别的 set 的按钮。
  **教训：架构转向最大的坑不在新代码，而在旧数据流以新形态流回旧消费者**（角度恒 0 这种
  "合法但从未出现过"的输入，把三年没炸过的相关系数计算炸了）。

### 路线图影响
- [x] 本次：offline_vision.py + API + 前端 + config 默认切换（阶段十）
- [ ] Phase B keypoint 微调后，离线分析直接吃新权重（改 [analysis] model 即可）
- [ ] 可选后续：录制停止后自动触发分析（`[analysis] auto_run`）；分析队列
- [ ] 9.3 数据扩充覆盖新泳池机位 → 压 ID 通胀

---

## 问题 #38：Phase B 训练验尸 — pose mAP 恒 0 的三层剥洋葱，与 17→19 点头的权重移植

### 背景
Tim 老师翻出 `job_4_annotations_2026_07_16.zip`（CVAT ultralytics YOLO pose 导出）问："Phase B 不是做了吗？"

对账结果，Phase B 的真实状态比 task.md 记录的**多做了两步、也多埋了两个雷**：
1. **标注确实完成了一半**：job_4 = 本机 CVAT 完成的 53 张 crop、19 点标注（与交接包 reference 集 stem 完全一致）；Emily 手里的另外 53 张尚未回收。
2. **训练也跑过**：`runs/pose/swimmer_pose_v1`（7-27，100 epochs，1.5h CPU）——box mAP50 0.911，**pose mAP50 全程 0.000**。关键点分支什么都没学到，静默失败后被搁置。它引用的 `swimmer_pose_local.yaml` 已不在，无法直接验尸。

### 三层剥洋葱（每层都有探针实证）

**第一层：数据被静默丢弃 25%。** CVAT 允许把骨架点标到 crop 边界外（肢体伸出裁剪框），"Ultralytics YOLO pose 1.0" 导出原样写出 <0 / >1 的坐标，ultralytics 数据校验直接把**整张图**判 corrupt——53 张丢 13 张。修复：`tools/sanitize_pose_labels.py`（越界点按"不可见"处理 x=y=v=0，bbox 裁回边界）。53 张全部回收。

**第二层：探针证伪"练得不够"假说。** 5 张图、关全部增广、40 epochs 过拟合：box mAP 0.995 / **pose mAP 0.0**。加大到 lr 0.01 × 133 步：pose_loss 纹丝不动。同环境 coco8-pose（17 点）40 epochs → pose mAP 0.645 ✅。同样的 5 张 crop 截成 17 点标签 → **pose mAP 0.995** ✅。二分结论：环境好、数据好、**"19 点"这一步坏**。

**第三层：真凶是 OKS 损失的梯度饱和。** ultralytics 关键点损失是 `1 - exp(-d²/(2σ)²/area)`。kpt_shape 从 17 变 19 → cv4 头必须全新初始化 → 初始预测离人体十万八千里 → d 巨大 → `exp(-e) ≈ 0` → **梯度消失，永远学不动**。17 点之所以能训，是因为预训练 cv4 的初始预测就落在人体附近，损失未饱和。从架构 yaml 重建（绕开 checkpoint 内嵌配置）同样失败——只要头是新的就死。

### 修复：头部权重移植（tools/seed_pose19_head.py）
把 17 点预训练 cv4 逐通道移植进 19 点头：前 51 通道（17 点 × xyv）原样拷贝，**脚尖的 6 个新通道用脚踝通道播种**（脚尖本来就长在脚踝旁边，起点误差几厘米而不是半张图）。cv4 中间层因 `c4 = max(ch/4, nk)` 也随 nk 变宽，行/列同样切片移植 + 播种；BatchNorm 统计量同步。

探针验证：**pose mAP 0 → 0.995**。梯度饱和假说实锤。

### 教训
1. **训练"成功跑完"≠训练成功**。7-27 那次 100 epochs 正常退出、box 指标漂亮，pose 指标 0 却没人看见。以后训练收尾必须看**每个任务头**的指标，pose 训练看 mAP50(P) 不是 mAP50(B)。
2. **改任务头形状 = 断掉预训练血脉**，而 OKS 型损失对"从零的头"是死刑。要么移植权重，要么换成先用 L2 warmup 的两阶段——移植更干净。
3. **探针（5 张图过拟合）是最便宜的训练管线体检**：1 分钟就能证伪"练得不够"，避免再烧 1.5 小时。三次探针的成本 < 一次盲目重训的 1/20。
4. CVAT 导出永远先过 sanitizer 再进训练（Emily 未来的导出同样适用）。

### 第四层洋葱（追加）：mosaic 增广饿死小数据姿态训练
v1 正式训练（mosaic 0.5 配方，为检测器时代调的）65 epoch 被早停：pose_loss 卡 ~9、val pose mAP 0。
并行对照实验（同数据同种子，仅关 mosaic/mixup）：pose_loss 80 epoch 降到 **4.8** 且未收敛，
val 上肉眼对比（`pose_verdict_80ep.jpg`）——**倒立/出水/垂直姿态的关键点已落到真实肢体上，
而 COCO 列几乎全部空白**。结论：mosaic 把 42 张单人 crop 拼成四格杂烩，本就稀缺的监督信号被
进一步稀释；小数据单人 crop 姿态微调必须关 mosaic。`tools/train_pose.py` 默认已改为
mosaic=0/mixup=0（--mosaic 可开回，用于未来的大规模多人帧数据）。

### 结果与部署（v2，2026-07-30）
- v2 训练 244 epochs 早停（约 1h CPU）。**部署 last.pt 而非 best.pt**——ultralytics 的 fitness
  由 box 指标主导，best.pt 的姿态头反而弱（best.pt 单独 val pose mAP 0.000 vs last.pt 0.016）。
  教训：**姿态任务的 checkpoint 选择不能信默认 fitness**，后续可用 save_period 或自定义 fitness。
- 肉眼验收（held-out val crop 三列图 `pose_verdict_v2_last.jpg`）：倒立 / 出水 / 垂直姿态的
  关键点全部落到真实肢体，COCO 列几乎全空白。**花游姿态第一次被模型"学会"**。
- 端到端（hybrid v2 检测器 + Phase B 姿态）在旧场地 set_008 全帧视频上成立：多人倒立
  腿部骨架同时准确描出（`set_008_phaseb_indomain.jpg`）。
- **推理裁剪几何必须匹配训练**：训练 crop 是"主体占 2/3 居中"，hybrid 原来贴检测框硬裁 →
  分布失配、conf 掉出门控。已在 HybridSwimmerDetector 中把 bbox 外扩 1.5× 再裁。
- **诚实边界（域差）**：Emily 新泳池的 set_013 上骨架仍大多缺失——53 张训练 crop 全部来自
  旧素材，新场地（水色/距离/压缩模糊）是 out-of-domain。与 detector v2 的"新场地 ID 通胀"
  同构。**解药是 9.3 数据扩充**：回收 Emily 的 53 张 + 用新泳池素材补标注重训。
- config.toml `[analysis] model = "runs/pose/swimmer_pose19_v2/weights/last.pt"` 已启用。

### 状态
- [x] 数据集 `data/training/phase_b/`：53 crop（42 train / 11 val 整段隔离），19 点 + flip_idx
- [x] 播种权重 `runs/pose/seed/yolov8s-pose19-seeded.pt`
- [x] v1（mosaic 配方）65 epoch 早停失败 → 对照实验定位 mosaic 问题（第四层）
- [x] v2 训练完成并部署（last.pt → [analysis] model）；hybrid 裁剪外扩 1.5×
- [ ] Emily 53 张回收 + 新泳池素材补标 → 重训（9.3，效果跨场地的关键）

---

## 问题 #39：标注包质检翻车 — "抽查两端"不是质检，以及泳池 crop 的肤色双峰

### 背景
给 Emily 的新泳池标注包 v1（94 crop）被 Tim 老师一眼打回："大部分都是地板砖，图片非常模糊。"复盘属实：v1 的质检只抽看了"最大的几张"（是运动员）和"预标注最多的几张"（发现了浮标并修了），**从未全量过目**——中间那批大块湿地砖误检全部漏网。

### 三个教训
1. **QA 只抽两端 = 没有 QA**。94 张缩略图拼一张 contact sheet 十秒就能看完，没有任何理由不全量目检。已把 contact sheet 固化为打包前的必经步骤。
2. **锐度排序会偏爱瓷砖缝**。拉普拉斯方差把"直线边缘密集"误当"清晰"——地砖 crop 的锐度得分反而最高。锐度只能在"确认是人"之后用。
3. **泳池场景最强的 crop 过滤器是肤色占比**：HSV 肤色像素比在垃圾（地砖/纯水花）上是 0.00~0.04%，在真人 crop 上从 0.32% 起跳，**天然双峰、零重叠**。一行 mask 解决了追踪静止过滤（地砖误检是闪烁的，抓不到）和检测置信度（v2 对湿地砖过度自信）都解决不了的问题。

### v2 管线（46 crop 全部人工目检通过）
追踪逐帧 → 面积 ≥7000px²（按新泳池框分布 p75 定，而不是拍脑袋）→ 肤色 ≥0.3% → 锐度排序 + 12 帧间隔 → contact sheet 全量目检 → 打包。38/46 带 Phase B 权重预标注（398 点，含脚尖）。

另：清晰度的天花板是素材本身（远机位 + DroidCam 压缩）。**治本在录制端：下次机位靠近 + 1080p。**

---

## 问题 #40：Phase B v3 成功了，瓶颈却"搬家"了 —— 以及两次被代理指标欺骗

### 一、Phase B v3：数据翻倍，效果 18 倍
总统大人亲手标完新泳池 46 张（CVAT 1.1 XML 导出，216 点/93 occluded），合并旧 53 张 →
99 张（81 train / 18 val，val 含新泳池 set_016 整段隔离）。用 #38 的胜出配方（播种权重 +
无 mosaic）训 159 epoch 早停。

**同一 val 集上的对照（决定性证据）**：

| 权重 | pose mAP50 | pose mAP50-95 |
|---|---|---|
| v2（仅旧 53 张） | 0.006 | 0.001 |
| **v3（99 张含新泳池）** | **0.110** | **0.057** |

18 倍提升，域差被填平了一大截。**再次踩到 #38 的坑**：best.pt（fitness 由 box 主导）
pose mAP 只有 0.055，而 last.pt 是 0.110 —— 姿态任务永远别信默认 checkpoint 选择。
已部署 `[analysis] model = runs/pose/swimmer_pose19_v3/weights/last.pt`。

### 二、瓶颈搬家：全帧管线仍然烂，但责任方变了
v3 部署后跑 Emily 新泳池 set_013 端到端，骨架依然稀稀拉拉。逐框诊断发现：

- 小框（74×41）上姿态置信度很高（13/19 点过阈值）
- **真正倒立运动员的大框（73×131）只有 1/19 点过阈值**
- f176 那个画面正中的倒立运动员，**检测器压根没给框**（conf 0.24，低于 0.35 门槛）

渲染检测框后真相大白：**高置信度的框全落在新泳池的红色泳道浮标上**。

### 三、两次被代理指标欺骗（本条最值钱）
为量化误检率，我用"肤色像素占比"当判据——**第一次翻车**：判据是 `H<25 或 H>165`，
而红色浮标正好落在这个区间，于是浮标被判成"真人"，算出"误检率 21%、提高阈值可解决"
的乐观结论。

修正判据（橙色调 H3-22 + 中等饱和 S50-160；实测浮标 0.00% vs 真人 19.4%）后重算，
结论**完全反转**：误检框中位置信度 0.59 > 真人框 0.36，提高阈值只会留下浮标、扔掉运动员。

但我不再信任何代理指标了——**直接抽 40 个检测框渲染出来用眼睛数**：按置信度排序，
**最高的 18 个（0.72~0.85）全部是红浮标，一个运动员都没有**；真运动员集中在 0.35~0.67。
目视误检率约 60%。

教训（与 #39 的"抽查两端不是质检"同源，但更狠一层）：
1. **代理指标必须先用已知正负样本验证**，否则它会把你带向反方向的结论——我差点建议
   "提高阈值"，那会让系统更糟。
2. **当结论会影响决策时，花十秒渲染出来看**。眼睛是唯一不需要被验证的判据。
3. **修好一个组件后，必须重测端到端**。Phase B 的 18 倍提升是真的，但用户看到的效果
   没变——因为瓶颈已经搬到了检测器。单组件指标 ≠ 产品效果。

### 四、detector v3：零额外标注成本的解法
关键发现：总统大人在 Phase B 质检时已经人工确认过那 46 张新泳池 crop 都是真运动员——
**它们的全帧 + 单人框就是现成的 Phase A 标注**，而且每一帧都含红浮标作为**未标注背景**，
正是教检测器"浮标不是人"所需的负样本。

`data/training/phase_a_v3/`：150 旧泳池帧 + 46 新泳池帧（196 张，139/57 整段隔离，
val = clip_portrait_2 旧 + set_016 新）。**从 v2 best.pt 热启动微调**而非从头训——
v2 当年的 350 帧素材已不在磁盘（`swimmer_det_local.yaml` 也已丢失），从头训会静默丢失
那部分知识。这也是一条教训：**训练数据要和权重一起进版本管理**。

### 五、detector v3 结果：双域双赢，端到端终于打通（2026-08-05）
69 epoch 早停（6.75h CPU @1280），best box mAP50 0.884。**分域评测证明没有拆东墙补西墙**：

| val 域 | v2 mAP50 | v3 mAP50 | v3 精确率 |
|---|---|---|---|
| 旧泳池 clip_portrait_2 (50帧) | 0.842 | **0.880** | 0.872 |
| 新泳池 set_016 (7帧) | 0.493 | **0.867** | **1.000** |
| 合计 (57帧) | 0.803 | **0.878** | 0.866 |

**40 框目视审计（与 #40 三、同方法）**：v3 抽样的 40 个框**全部是真运动员**，零浮标零地砖
（v2 是"最高置信度的 18 个全是浮标"）。总框数 114 → 61 也印证了 v2 约 60% 是垃圾：
v2 的 114 框里真人约 46 个，**v3 的 61 框全是真人——真检出反而多了 33%**。

新泳池召回从 0.987 掉到 0.693 看似退步，实则是 v2 的"高召回"建立在精确率 0.463 之上
（宁滥勿缺）。产品语境下**精确率优先**：漏检只是某几帧没骨架（25fps 下无伤大雅），
误检却是把浮标画成人——正是总统大人最初投诉的现象。

**端到端验收**（hybrid v3 检测器 + v3 姿态，34 秒跑完 294 帧）：
- 新泳池 set_013：216/294 帧有骨架，track ID 21 → 13；放大目检确认骨架落在运动员身上，
  **倒立姿态的绿色脚尖点正确出现在脚趾**（花游绷脚评分的核心指标）
- 旧泳池 set_008 回归测试：多人倒立腿部骨架依旧准确，**零退化**

### 状态
- [x] Phase B v3 训练完成并部署（pose mAP 0.006 → 0.110）
- [x] detector v3 训练完成并部署（新泳池 mAP 0.493 → 0.867、精确率 → 1.000；旧泳池同步提升）
- [x] 端到端双域验收通过，config 已启用 hybrid（det v3 + pose v3）
- [ ] 下一轮增量：Emily 新录制（机位靠近 + 1080p）→ 继续转飞轮

---

## 问题 #41：把模型送到 Emily 手上 —— kit 打包链路的三个隐性断点

### 背景
v3 双模型验收通过后要打 kit 发给 Emily。动手前先审打包链路，发现**三个会让她拿到"看起来更新了、其实没生效"的包**的断点——每一个单独都足以让这次交付白做。

### 断点一：`git archive HEAD` 打不到未提交的代码
kit v3（PR #15）用 `git archive HEAD` 生成 code.zip。而这几天所有修复（录制/推理解耦、
`offline_vision.py`、19 点映射、config）**全部未提交**——按原逻辑打出来的包会是几天前的旧代码，
Emily 双击更新后一切照旧，而且**没有任何报错**。

改为打**工作树**：`git ls-files`（自动尊重 .gitignore）+ 显式补上运行时必需但尚未纳入
版本管理的文件（`fastapi_app/offline_vision.py`、`data/tracker_configs/`）。
Emily 拿到的必须是我们实测过的那一份，而不是"碰巧提交了的那一份"。

### 断点二：上线权重的路径只在训练机上存在
config 原本指向 `runs/detect/swimmer_det_v3/weights/best.pt` —— `runs/` 在 .gitignore 里，
**只存在于做训练的那台电脑**。Emily 的安装里这个路径永远是空的。

引入 `models/` 作为**部署权重的规范位置**：开发机上是指向 `runs/…` 的硬链接（不占额外磁盘，
当时磁盘只剩 3.6G），kit 里是真文件。config 统一写 `models/swimmer_det_v3.pt`，两边同构。
配套：kit 分 `models/`（通用底座 → 仓库根，不覆盖）和 `vision_models/`（自训权重 → 仓库
`models/`，**必须覆盖**）；文件名自带版本号，覆盖不会误伤任何训练产物。
原来的 `cp -n` 对自训权重是错的——那意味着新模型永远装不上。

### 断点三：更新会抹掉 Emily 的本机设置
`unzip -o code.zip` 整体覆盖 config.toml —— 这正是新模型路径生效的方式，但也会把她的
摄像头地址、旋转角度一起重置。（这解释了她那台机器为何一直跑默认 `yolo_device="mps"`，
而 Intel Mac 根本没有 Metal。）

方案：**分层配置**。`config.toml` 走版本管理、随 kit 覆盖；`config.local.toml` 只属于这台
电脑、永不进包，深度合并覆盖在上面（`[hardware] camera_url` 只覆盖这一个键，同表其他键保留）。
`update.command` 首次更新时自动把旧 config 里的机器相关项迁进去，并按 `uname -m`
自动判定设备（x86_64 → cpu，arm64 → mps）—— Emily 不需要理解这件事。
坏的本地文件只打印警告不致命：一个覆盖文件写错了不该让整个 dashboard 起不来。

### 验证：不能只看"包打出来了"
把 kit 解到一个模拟目录，**复刻 Emily 的真实处境**（旧 config 带她自己的
`192.168.1.77` 摄像头 + 一个假的 `runs/` 训练产物），跑完整更新流程后断言：
- 她的 camera_url / rotation **原样保留**（进了 config.local.toml）
- 两个 v3 权重就位、`resolve_analysis_config()` 能解析到
- `runs/` 训练产物没被动、根目录旧的 `yolov8s-pose.pt` 没被误覆盖
- code.zip 里 `camera_manager.py` 含 `_infer_loop`（证明打的是工作树不是 HEAD）
- config.local.toml 没有混进包里

另加 10 条 `tests/test_config_local.py` 锁住合并语义（含"显式传 config_path 的测试
不得读到开发机的本地覆盖"——否则测试结果会随开发机环境漂移）。

### 教训
1. **交付链路和被交付物一样需要测试**。模型指标再漂亮，打包环节漏一个文件，用户侧收益为零。
2. **"静默地不生效"是最贵的失败模式**：`git archive HEAD`、`cp -n`、路径只在本机存在——
   三者都不报错，只是让更新变成一场空。
3. **配置要分层**：机器相关 vs 版本相关必须物理隔开，否则"更新"和"保留个性化"永远二选一。
4. 面向远端非技术用户的交付，**必须在本地模拟一遍他们的处境**——他们无法 debug，
   也没法告诉你哪里错了。

---

## 问题 #42：两条并行开发线合流 —— 上游换了数据，下游还站得住吗

### 背景
#36~#41 这条线（离线视觉 + Phase B/A v3）与 PR #16 的 Choreography Intelligence（#35）
是并行开发的，合并时 main 已分叉。关键在于二者不是无关的并行工作，而是**上下游**：
#16 的全部计算都读 `landmarks_multi.jsonl`，而那正是我们这条线重做的东西。
"能合上"不等于"合了还对"——rebase 之后必须验证下游仍然成立。

### 合并本身的三处撞车
1. **DEVLOG 编号撞车**：双方都写了"问题 #35"。#16 先落地，保留其 #35；我们整体后移为
   #36~#41，并同步修正 23 个文件里的 `DEVLOG #NN` 交叉引用（含 `#37 的坑` 这类裸引用）。
   降序替换避免连锁改错；`docs/research-roadmap.md`（#16 的文件）里的 `#35` 严格保留。
2. **task.md 阶段号撞车**：双方都新建"阶段十"。#16 的研究方向保留阶段十，我们的架构转向
   顺延为阶段十一（其下 10.x → 11.x）。
3. **api_routes.py 同位置新增端点**：两组端点全部保留，路由无重复（32 条，`/analyze*`
   与 `/choreography*` 各自独立）。

### 兼容性验证：同一段视频，两代数据喂给同一个引擎
`set_013`（Emily 新泳池）——她录制时的原始 landmarks（v2 检测器 + COCO 姿态）
vs 我们用 v3 双模型离线重分析后的 landmarks：

| #16 的变量 | Emily 原始 (v2+COCO) | v3 重分析 | 说明 |
|---|---|---|---|
| movement_frequency | **算不出 (None)** | 0.965 Hz | 从无到有 |
| rotation_frequency | **算不出 (None)** | 15.50 deg/s | 从无到有 |
| leg_angle_deviation | 26.93° | **1.69°** | 论文基准 5.34°(intl)；26.9° 是物理上不可能的幻觉值 |
| leg_dev 可测腿数 | 16 | **90** | 5.6× |

结论：**#16 的引擎不但没被破坏，反而第一次拿到了能用的输入**。三个变量里两个从
"数据不足算不出"变成有值；腿部偏差从荒谬的 26.9° 回到物理合理区间——那 26.9° 正是
COCO 幻觉骨架的直接后果（腿被画到任意方向），也从侧面印证了 #40 的结论。
#16 自带的 13 个几何单元测试全过；全套 129 passed（16 个失败为既有）。

### 一处刻意不动的不一致
阈值链条目前是三档：源头门控 `kpt_min_conf=0.35` → 前端画线 0.45 →
#16 测量 `VIS_THRESHOLD=0.5`。看着像不一致，其实是合理的分层——**测量应当比可视化更严格**。
统一它会让 #16 已验证的数值全部失效，不能为了"看起来整齐"去动一个已通过对抗验证的常数。
记录在此，等有真实需求（如 leg_angle_deviation 覆盖不足）再作为一个独立实验来调。

### 教训
**合并不是终点，是新的验证起点。** 上游改了数据格式/分布，下游代码即使一行没动、
测试全绿，也可能已经在算错的东西。判据不是"跑不跑得起来"，而是
"**在真实数据上，它给出的数还成立吗**"——这次是拿同一段视频喂两代数据做的对照。
